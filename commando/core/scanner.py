...
from commando.utils.io import (
    ...
    save_json, load_json, clear_screen, pause, BASE_DIR
)
...
def auto_scan_system(state_manager):
    ...
    def scan_worker(cmd):
        ...
        try:
            ...
            return cmd, {"desc": sanitize_text(desc), "example": f"{cmd} --help", "category": "Auto-Imported Libs", "source": "OS Manual"}
        ...
        try:
            ...
            desc = process.stdout.strip()
            if desc:
                return cmd, {"desc": sanitize_text(desc), "example": f"{cmd} --help", "category": "Auto-Imported Libs", "source": "Built-in"}
        except (subprocess.CalledProcessError, FileNotFoundError):
            pass
